...
namespace UnityStandardAssets.Characters.FirstPerson
{
    ...
    public class RigidbodyFirstPersonController : MonoBehaviour
    {
        ...
        public class MovementSettings
        {
            ...
            public float RunMultiplier = 2.0f;   // Initial Speed when sprinting
            public float MaxRunMultiplier = 4.0f; // Max Speed when sprinting
...
#if !MOBILE_INPUT
	            if (Input.GetKey(RunKey) && input.y > 0)
	            {
		            CurrentTargetSpeed *= RunMultiplier;
		            m_Running = true;
                    if (RunMultiplier < MaxRunMultiplier)
                    {
                        RunMultiplier += 2 * Time.deltaTime;
                    }        
	            }
	            else
	            {
